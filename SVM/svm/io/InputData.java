...
public class InputData extends Dialog {
	...
	void loadInputData(File file) {
		...
		if (metadata != null) {
			...
			for (int i = 0; i < metadata.length; i++) {
				if (metadata[i].startsWith("@relation") || metadata[i].startsWith("@data"))
					continue;
				...
				if (info.startsWith("@attributeclass")) {
					...
					if (ss.length == 2) {
						...
						for (int j = 0; j < ss.length; j++)
							classes[j] = new Clasa(ss[j], j + 1, new Color(128 + (int) (Math.random() * 128),
									128 + (int) (Math.random() * 128), 128 + (int) (Math.random() * 128)));
					}
					continue;
		...
		for (int i = 0; i < data.length; i++) {
			...
			for (int u = 0; u < classes.length; u++)
				if (classes[u].name.startsWith(coords[coords.length - 1].trim())) {
					t = u;
					break;
				}
			for (int j = 0; j <= coords.length - 2; j++) {
				try {
					X[j] = Float.parseFloat(coords[j]);
				} catch (NumberFormatException e) {
					System.out.println("The vector " + k + " is not correct!");
					break;
				}
			}
			if (t > -1) {
		...
		if (comments != null)
			for (int i = 0; i < comments.length; i++)
				ta.append(comments[i] + "\n");
		if (metadata != null)
			for (int i = 0; i < metadata.length; i++)
				ta.append(metadata[i] + "\n");
		if (data != null)
			for (int i = 0; i < data.length; i++)
				ta.append(data[i] + "\n");
		svm.control.show();
	...
	public void split() {
		...
		if (T != null)
			svm.outd.testing_vectors_count = T.length;
		init();
	}
	...
	public void getStatistics() {
		for (int i = 0; i < attributes.length; i++) {
			...
			for (int j = 0; j < classes.length; j++) {
				attributes[i].class_statistic[j].statistic_name = attributes[i].attribute_name + ": Class "
						+ classes[j].Y;
