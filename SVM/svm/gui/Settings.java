...
public class Settings extends Dialog {
	...
	void saveSettings() {
		...
		try {
			axis_min_ = Integer.parseInt(axis_min_tf.getText());
		} catch (NumberFormatException e) {
		}
		try {
			axis_max_ = Integer.parseInt(axis_max_tf.getText());
		} catch (NumberFormatException e) {
		}
		if (axis_min_ < axis_max_) {
			axis_min = axis_min_;
		...
		try {
			...
			if (0 <= r && r <= 255 && 0 <= g && g <= 255 && 0 <= b && b <= 255) {
				class0_point_color = new Color(r, g, b);
				if (svm.ind.classes != null)
					svm.ind.classes[0].color = class0_point_color;
			}
		} catch (NumberFormatException e) {
		}
		try {
			...
			if (0 <= r && r <= 255 && 0 <= g && g <= 255 && 0 <= b && b <= 255) {
				class1_point_color = new Color(r, g, b);
				if (svm.ind.classes != null)
					svm.ind.classes[1].color = class1_point_color;
			}
		} catch (NumberFormatException e) {
		}
		try {
		...
		try {
			...
			bw.write("background_color=" + background_color.getRed() + " " + background_color.getGreen() + " "
			...
			bw.write("class0_point_color=" + class0_point_color.getRed() + " " + class0_point_color.getGreen() + " "
					+ class0_point_color.getBlue() + "\n");
			bw.write("class1_point_color=" + class1_point_color.getRed() + " " + class1_point_color.getGreen() + " "
					+ class1_point_color.getBlue() + "\n");
			bw.write("line_color=" + line_color.getRed() + " " + line_color.getGreen() + " " + line_color.getBlue()
					+ "\n");
			bw.write("string_color=" + string_color.getRed() + " " + string_color.getGreen() + " "
					+ string_color.getBlue() + "\n");
