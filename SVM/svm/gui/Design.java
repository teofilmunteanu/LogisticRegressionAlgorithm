...
public class Design extends Panel implements MouseListener, MouseMotionListener {
	...
	public void drawAxis(Graphics g) {
		...
		if (svm.settings.axis) {
			...
			if (svm.settings.gradations) {
				for (int i = Ox; i <= Ox + svm.settings.axis_max; i += svm.settings.axis_gradations)
					g.drawLine(i, Oy - 2, i, Oy + 2);
				for (int i = Ox; i >= Ox + svm.settings.axis_min; i -= svm.settings.axis_gradations)
					g.drawLine(i, Oy - 2, i, Oy + 2);
				for (int j = Oy; j <= Oy + svm.settings.axis_max; j += svm.settings.axis_gradations)
					g.drawLine(Ox - 2, j, Ox + 2, j);
				for (int j = Oy; j >= Oy + svm.settings.axis_min; j -= svm.settings.axis_gradations)
					g.drawLine(Ox - 2, j, Ox + 2, j);
			}
		}
	}
	...
	public void mouseMoved(MouseEvent me) {
		...
		if (ccx <= 2 || ccx >= ww - 5 || ccy <= 5 || ccy >= hh - 5)
			coords = "";
		repaint();
	}
